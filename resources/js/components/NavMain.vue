...
onMounted(() => {
  for (const item of props.items) {
    if (page.url?.includes(item.prefix ?? 'none')) {
  ...
  <SidebarGroup class="px-2 py-0">
    ...
    <SidebarMenu class="font-medium cursor-pointer">
      <SidebarMenuItem v-for="(item) in items" :key="item.title">
        <SidebarMenuButton size="lg" v-if="item.subMenu" as-child
          :disabled="item.restrictedUserType?.includes(user.user_type as UserType)"
          :is-active="isSubMenuOpen(item.title) || page.url?.includes(item.prefix ?? 'none')" :tooltip="item.title"
        ...
        <SidebarMenuButton size="lg" v-else as-child :is-active="item.href?.includes(page.url)" :tooltip="item.title">
          <template v-if="!item.restrictedUserType?.includes(user.user_type as UserType)">
          ...
          <template v-else>
            <div class="flex items-center gap-2 select-none w-full cursor-not-allowed">
              <Icon v-if="item.tIcon" class="tIcon text-muted-foreground" :icon="'tabler:' + item.tIcon" />
        ...
        <Transition enter-active-class="transition-all duration-300 ease-in-out" enter-from-class="opacity-0 max-h-0"
          ...
          <SidebarMenuSub v-if="item.subMenu && isSubMenuOpen(item.title)" class="select-none">
            <SidebarMenuSubItem v-for="(sub, idx) in item.subMenu" :key="sub.title + idx">
              <SidebarMenuSubButton class="py-2 h-full" :is-active="sub.href?.includes(page.url)">
                <!-- If the user is not restricted, show the link -->
                <template v-if="!sub.restrictedUserType?.includes(user.user_type as UserType)">
                ...
                <!-- If the user is restricted, show the lock icon -->
                <template v-else>
                  <div class="flex items-center gap-2 select-none w-full cursor-not-allowed">
                    <Icon v-if="sub.tIcon" class="tIcon text-muted-foreground" :icon="'tabler:' + sub.tIcon" />
                    <span class="text-muted-foreground">{{ sub.title }}</span>
                    <Icon icon="tabler:lock" class="h-4 w-4 text-muted-foreground" />
                  </div>
                </template>
