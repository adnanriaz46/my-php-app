...
watch(successMessage, (msg) => {
  if (msg) {
    showToast('My List', msg as string, 'success')
  }
}, {deep: true});
...
const myPropertyList = ref<SelectBoxOption[]>([])
...
const fetchInitialData = async () => {
  ...
  myPropertyList.value = res.data?.map(value => {
    return {key: value.id, value: value.name + ` (${value.property_ids.length})`}
  });
}

const submit = () => {
  if (!addAsNew.value) {
    updateForm.selectionIds = props.selectionIds;
    updateForm.post(route('property.my_list.update_property'), {
      onSuccess: () => {
        updateForm.reset()
        fetchInitialData();
    ...
    createForm.selectionIds = props.selectionIds;
    createForm.post(route('property.my_list.create_property'), {
      onSuccess: () => {
        createForm.reset()
        fetchInitialData();
        dialogOpen.value = false;
  ...
  <Dialog v-model:open="dialogOpen">
    <DialogContent>
      <DialogHeader class="space-y-3">
        ...
        <DialogDescription>
          Add selected {{ selectionIds.length }} properties to your list
          <InputError v-if="updateForm.errors.selectionIds" :message="updateForm.errors.selectionIds"/>
          <InputError v-if="createForm.errors.selectionIds" :message="createForm.errors.selectionIds"/>
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter class="gap-2">
        ...
        <Button @click="submit" variant="default" :disabled="updateForm.processing || createForm.processing">
          Save
        </Button>
      </DialogFooter>
    </DialogContent>
