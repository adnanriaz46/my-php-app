...
import {
  ...
  getGeoAddressInfo, GeoAddressObject, getGeoDataToAddressObject
...
const props = defineProps<{
  address?: GeoAddressObject | null
  class?: string
}>();

const emit = defineEmits<{
  (e: 'update:address', value: GeoAddressObject): void;
}>();

watch(
  () => props.address,
  (newAddress) => {
    if (newAddress && newAddress.geoAddress) {
      searchQuery.value = newAddress.geoAddress;
    } else {
      searchQuery.value = '';
    }
  },
...
const AddressObject = ref<GeoAddressObject>();
const updateSuggestions = () => {
  fetchPlaceAddressSuggestions()
}

const selectSuggestion = async (item: string) => {
  AddressObject.value = undefined;
  suggestions.value = [];
  isFocused.value = false;

  const data = await getGeoAddressInfo(item);
  if (data && data[0]) {
    AddressObject.value = getGeoDataToAddressObject(data[0]);
    if (AddressObject.value && AddressObject.value.geoAddress) {
      searchQuery.value = AddressObject.value.geoAddress;
    }
    emit('update:address', AddressObject.value);
  }
}
...
const onArrowUp = () => {
  ...
  highlightedIndex.value =
    highlightedIndex.value <= 0 ? total - 1 : highlightedIndex.value - 1
}
...
<template>
  <div class="relative w-full">
    ...
    <Input
      ...
      placeholder="Search Address" class="pl-10 border-gray-300 shadow-sm dark:border-gray-700 h-[40px]" :class="props.class" />
